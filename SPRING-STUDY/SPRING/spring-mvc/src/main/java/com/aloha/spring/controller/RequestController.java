...
//	@RequestMapping("/board")
	...
	public String requstPath(@PathVariable("no") int no) {
	...
	public String header(@RequestHeader("Accept") String accept
				...
				String requestUserAccept = request.getHeader("Accept");
				logger.info("@RequestHeader 객체로부터 헤더 가져오기");
				logger.info("Accept - " + accept);
				logger.info("User-Agent - " + userAgent);
				return "SUCCESS";
		}
	 ...
	 *  
	 *  * 비동기 또는 thunder cliest 로 테스트 가능
...
//		FileCopyUtils.copy(파일 데이터, 파일 객체); 	// 파일 업로드
	...
	public String fileUpload(@RequestParam MultipartFile[] fileList) throws Exception {
...
//		FileCopyUtils.copy(파일 데이터, 파일 객체); 	// 파일 업로드
	...
	public String filrUpload(Board board) throws Exception {
...
//		FileCopyUtils.copy(파일 데이터, 파일 객체);
	...
	public String filrUploadUploadPost(Board board) throws Exception {
		logger.info("/request/file/board");
		logger.info("uploadPath : " + uploadPath);
		logger.info("board : " + board);
